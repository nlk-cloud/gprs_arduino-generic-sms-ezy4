          ...
          Serial.println(" Entered delay");
            ...
            if (entiremsg.indexOf("ON") > 0)
            {
                if (entiremsg.indexOf("MIN") > 0)
                {
                    ...
                    inputondelay[p]= (inputondelay[p]|(0x80)); // Setting MSB of one byte to indicate units is Min
                ...
                offsetaddr = 215 + (p * 2);
                writeUnsignedIntIntoEEPROM(offsetaddr, inputondelay[p]);
                inputondelay[p] = readUnsignedIntFromEEPROM(offsetaddr);
            }
            ...
            if (entiremsg.indexOf("OFF") > 0)
            {
              Serial.println(" Entered off delay");
                if (entiremsg.indexOf("MIN") > 0)
                {
                    ...
                    inputoffdelay[p]= (inputoffdelay[p]|(0x80)); // Setting MSB of one byte to indicate units is Min
                ...
                offsetaddr = 223 + (p * 2);
                writeUnsignedIntIntoEEPROM(offsetaddr, inputoffdelay[p]);
                inputoffdelay[p] = readUnsignedIntFromEEPROM(offsetaddr);
            }
        response = 16;
        SendResponse();
        break;
