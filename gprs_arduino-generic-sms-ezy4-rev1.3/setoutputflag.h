                    ...
                    if (toname.length()==0)
                       {
                          ...
                          outputstatusflag[p]=outputstatusflag[p]|(0x40);
...
// come back to 1***/
void driveoutputonoff( uint8_t r, uint8_t status) // r is the output no. and status is what needs to be driven on that output
{
   ...
   outputstarted[r]=1;   // Flag to say that output has started

   m=outputstatusflag[r]&(0x03); // check the units of delay - mins or hrs or days
   
   switch (m)
    {
     // No time is specified. It is just 'TURN ON'
        case 0:    
    ...
    // time specified is in mins
        case 1:
         ...
         outputdlyattained[r]=dly(currtime[r],0,0,outputontime[r],0); //timekeeping.h 
    ...
    // time specified is in hrs
        case 2:
          ...
          outputdlyattained[r]=dly(currtime[r],0,outputontime[r],0,0); //timekeeping.h 
    ...
    // time specified is in days
        case 3:
          ...
          outputdlyattained[r]=dly(currtime[r],outputontime[r],0,0,0); //timekeeping.h 
